...
(function() {
    ...
    document.addEventListener("DOMContentLoaded", function() {
        clearModalInputs();
        urlroot = $("#urlroot").val();
        document.querySelector("#"+ids.add_question_btn).addEventListener("click", () => {
             requestHandler();
        });
    ...
    function requestHandler() {
        ...
        if(errors.assessment_id_err.length == 0 && errors.section_id_err.length == 0 && errors.question_statement_err.length == 0 && errors.option_a_err.length == 0 && errors.option_b_err.length == 0 && errors.option_c_err.length == 0 && errors.option_d_err.length == 0 && errors.correct_option_err.length == 0 && errors.question_reasoning_err.length == 0) {
            ...
            var ajax = new AjaxCall(urlroot +"/questions/addQuestion", "POST", inputs, "json");
            ajax.ajaxCall()
            .then((response) => {
                ...
                switch (response.status) {
                    ...
                    case "success":
                        scrollToTopOfWindow();
                        clearModalInputs();
                        showErrorMessage("New question added successfully!", errorIds.success_message);
                        break;
                    default:
                        break;
                }
    ...
    function clearModalInputs() {
        $("#"+ids.question_statement).val("");
        $("#"+ids.option_a).val("");
        $("#"+ids.option_b).val("");
        $("#"+ids.option_c).val("");
        $("#"+ids.option_d).val("");
        $("#"+ids.correct_option).val($("#" + ids.correct_option + " option:first").val());
        $("#"+ids.question_reasoning).val("");
        
        $("#"+errorIds.assessment_id_err).text("");
        $("#"+errorIds.category_id_err).text("");
        $("#"+errorIds.question_statement_err).text("");
        $("#"+errorIds.option_a_err).text("");
        $("#"+errorIds.option_b_err).text("");
        $("#"+errorIds.option_c_err).text("");
        $("#"+errorIds.option_d_err).text("");
        $("#"+errorIds.option_d_err).text("");
        $("#"+errorIds.correct_option_err).text("");
    }
